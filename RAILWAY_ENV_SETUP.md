...
1. **OPENAI_API_KEY** ⚠️ **必須設定**
   - OpenAI APIキー（バックエンドで使用）
   - Railwayダッシュボードの「Variables」タブで設定
   - **重要**: この環境変数が設定されていないと、API呼び出しが500エラーになります
...
### APIキーが未設定と表示される場合（500エラー）

1. **環境変数が設定されているか確認**
   ```bash
   railway variables
   ```
   または、Railwayダッシュボードの「Variables」タブで確認

2. **`OPENAI_API_KEY`が設定されているか確認**
   - Railwayダッシュボードの「Variables」タブで`OPENAI_API_KEY`が存在するか確認
   - 値が正しく設定されているか確認（`sk-proj-`で始まる必要があります）

3. **環境変数を再設定**
   - Railwayダッシュボードの「Variables」タブで`OPENAI_API_KEY`を追加または更新
   - 値: `sk-proj-...`（実際のOpenAI APIキー）

4. **再デプロイを実行**
   - Railwayダッシュボードで「Redeploy」をクリック
   - または、新しいコミットをプッシュ
   - 再デプロイが完了するまで待つ（通常2-5分）

5. **ログを確認**
   - Railwayダッシュボードの「Logs」タブでエラーログを確認
   - 「OpenAI API key not found」というログが表示されていないか確認
